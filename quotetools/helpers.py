...
def role_mention_cleanup(message: discord.Message) -> Union[str, None]:
    ...
    assert isinstance(content, str), "message.content ist irgendwie durcheinander geraten..."  # nosec
...
def embed_from_msg(message: discord.Message) -> discord.Embed:
    ...
    if message.attachments:
        ...
        if fname.split(".")[-1] in ["png", "jpg", "gif", "jpeg"]:
            ...
            em.add_field(
                name="Nachricht hat einen Anhang", value=f"[{fname}]({url})", inline=True
            )
    em.add_field(
        name="Scheint dies aus dem Zusammenhang gerissen zu sein?",
        value=f"[Klicken, um zur ursprünglichen Nachricht zu springen]({message.jump_url})",
        inline=False,
    )
    return em
...
async def eligible_channels(ctx: commands.Context) -> Sequence[discord.TextChannel]:
    """
    Hol dir die förderfähigen Kanäle zum Prüfen
    """
